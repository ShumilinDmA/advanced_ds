...
def gini_score_pandas(subset: pd.DataFrame, y_true_col: str = "y_true", y_pred_col: str = "y_pred", **kwargs) -> float:
    """
    ...
    :param kwargs: Params for gini_score
    :return: gini score
    """
    y_true = subset[y_true_col]
    y_pred = subset[y_pred_col]
    return gini_score(y_true, y_pred, **kwargs)


def pr_auc_score_pandas(subset: pd.DataFrame, y_true_col: str = "y_true", y_pred_col: str = "y_pred",
                        **kwargs) -> float:
    """
    ...
    :param y_true_col: name of column which contain true labels
    :param y_pred_col: name of column which contain predictions for class 1
    :param kwargs: Params for pr_auc_score
    :return: pr auc score
    """
    y_true = subset[y_true_col]
    y_pred = subset[y_pred_col]
    try:
        auc_val = pr_auc_score(y_true, y_pred, **kwargs)
    except ValueError:
        auc_val = 0
    return auc_val
...
def precision_at_recall_auc_score_pandas(subset: pd.DataFrame, recall_threshold: float,
                                         y_true_col: str = "y_true", y_pred_col: str = "y_pred", **kwargs) -> float:
    """
    Wrapper to get precision recall area under curve for each group of data
    ...
    :param y_true_col: name of column which contain true labels
    :param y_pred_col: name of column which contain predictions for class 1
    :param kwargs: Params for precision_at_recall_auc_score
    :return: area under shorted precision recall
    """
    y_true = subset[y_true_col]
    y_pred = subset[y_pred_col]
    return precision_at_recall_auc_score(y_true, y_pred, recall_threshold, **kwargs)
...
def adjusted_r2_score_pandas(subset: pd.DataFrame, n_features: int,
    ...
    :param y_true_col: name of column which contain true labels
    :param y_pred_col: name of column which contain predictions for class 1
    :param kwargs: Params for adjusted_r2_score_pandas
    :return: adjusted r2 score
    """
    y_true = subset[y_true_col]
    y_pred = subset[y_pred_col]
    return adjusted_r2_score(y_true, y_pred, n_features=n_features, **kwargs)
