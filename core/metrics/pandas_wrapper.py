...
def adaptive_expected_calibration_error_pandas(subset: pd.DataFrame, y_true_col: str = "y_true", y_pred_col: str = "y_pred",
                                      bins: int = 10) -> float:
    """
...
def mean_calibration_error_pandas(subset: pd.DataFrame,
                                              y_true_col: str = "y_true", y_pred_col: str = "y_pred") -> float:
    """
    Wrapper to adECE score for each group of data
    example: df.groupby(by="something").apply(alexander_wanted_calibration_error_pandas,
    ...
    y_true = subset[y_true_col]
    y_pred = subset[y_pred_col]
    return mean_calibration_error(y_true, y_pred)